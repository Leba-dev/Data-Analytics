...
-- Version Subqwery en el FROM
SELECT temporary_table.id, temporary_table.name, temporary_table.surname, temporary_table.transactions_count
FROM (
    SELECT u.id, u.name, u.surname, COUNT(t.id) AS transactions_count
    FROM users u
    JOIN transactions t 
...
WHERE temporary_table.transactions_count > 30
ORDER BY temporary_table.transactions_count DESC;
